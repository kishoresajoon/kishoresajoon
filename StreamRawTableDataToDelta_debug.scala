...
val tableList = cdcTableList.get(priorityOfTableList).get.map{case(k,v) => k.toUpperCase -> v}.filter{
  case (tableName, _) => tableName == "PMTDTA.DACTPF"
}
val tableNames = tableList.keySet.map(_.toUpperCase)
...
object CDCMerge {
  ...
  private def getMergeWithKeys(inputDF: DataFrame, metadata: MetadataPayload, partitionNum: Integer): MergeWithKeys = {
    println("reach getMergeWithKeys")
    ...
    val outputDF = withTypesDF.coalesce(partitionNum)
...
def processDataForSchemaId(batchId: Long, batchDF: DataFrame, metadata: MetadataPayload, partitionNum: Integer): Unit = {
  println("reach processDataForSchemaId")
...
def processDataForTable(batchId: Long, batchDF: DataFrame, metadataArray: Array[MetadataPayload], partitionNum: Integer): Unit = {
  println("reach processDataForTable")
...
// validation run for DACTPF
val sample_tbp = spark.read.format("delta").load(dataTableRawPath).where("body.messageSchemaId = '0BB38F0824B91548EAB4AE0F2AAEBDE7'").limit(10)

// COMMAND ----------

upsertToDelta(sample_tbp,1234231)

// COMMAND ----------

display(spark.read.format("delta").load(metadataStructuredTablePath).where("table = 'CHDRPF'"))

// COMMAND ----------

display(spark.read.format("delta").load(metadataStructuredTablePath).where("messageSchemaId = '47E17A7D7E18A3A20EE2033F188932AC'"))

// COMMAND ----------

display(spark.read.format("delta").load(metadataStructuredTablePath).where("messageSchemaId = '0BB38F0824B91548EAB4AE0F2AAEBDE7'"))

// COMMAND ----------

val schemaIds = Seq("0BB38F0824B91548EAB4AE0F2AAEBDE7")
GetCurrentMetadata(metadataSchemaIds = schemaIds)

// COMMAND ----------

// using chdrpf
val sample_tbp = spark.read.format("delta").load(dataTableRawPath).where("body.messageSchemaId = '47E17A7D7E18A3A20EE2033F188932AC'").limit(10)

// COMMAND ----------

// upsert disect 
val batchDF = sample_tbp
val batchId = 1234231
println(s"Batch: ${batchId}")
...
//   }
// }
val tableMeta = metaParhHashkv.head
val table = tableMeta.table
val index = tableMeta.index
val metadataSortedArray = tableMeta.metaArray
...
// batchId, batchDF, metadataSortedArray, partitionNum = schedulerPoolPartitionNum)
 
val metadataArray=  metadataSortedArray 
val partitionNum = schedulerPoolPartitionNum
println("reach processDataForTable")
 metadataArray.foreach {
   metadata => {
     println("Starting method processDataForSchemaId : " +metadata)
...
// processDataForSchemaId(batchId: Long, batchDF: DataFrame, metadata: MetadataPayload, partitionNum: Integer): Unit = {
val metadata = metadataSortedArray.head
val partitionNum = 123445
println("reach processDataForSchemaId")
val startLogDF = getLogInfo(metadata.tableSchema, metadata.tableName, metadata.messageSchemaId,"delta",Timestamp.valueOf(java.time.LocalDateTime.now),"processDataForSchemaId: Start",logLevel)
saveLogInfo(startLogDF, logLevel)
...
// ef convertToIBMColumnHandleDataTypeMismatch(tableSchema: String, tableName: String, columns: Seq[ColumnDetails]): Seq[IBMDB2ColumnV2] = { 

val tableSchema = metadata.tableSchema
val tableName   = metadata.tableName
val columns     = metadata.allColumns
...
allCDCcolIBM2.foreach{x => println(x)}

// COMMAND ----------

piiColumnsIBMDB2SeqConfig.get("DACTPF").getOrElse(Nil)
...
strcutredLayerSchema.foreach{x => println(x)}
...
allCDCcolumns.map(allCDCcolIBM2map)

// COMMAND ----------

// MAGIC %md ### END Dissect
